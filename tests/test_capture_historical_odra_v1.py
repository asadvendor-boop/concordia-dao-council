...
import scripts.capture_historical_odra_v1 as capture_module
...
from tests.test_casper_rpc_transport import _Response, _client
...
class _FakeRpc:
    ...
    def call(
        ...
        *,
        ...
        if (
            self._corrupt_second
            and endpoint == NODE_B
            and method == "query_global_state"
        ):
...
def test_capture_uses_two_read_only_nodes_and_receipt_selected_card_prefix(
    ...
    assert (
        facts["deployHash"]
        == fixture["raw_rpc"]["deploy"]["request"]["params"]["deploy_hash"]
    )
...
def test_capture_fails_closed_when_second_public_node_disagrees(
    ...
    with pytest.raises(
        HistoricalCaptureError, match="second public RPC|disagree|invalid"
    ):
        capture_historical_odra_v1(
            proposal_payload=_proposal_payload(fixture["card_chain"]),
...
def test_historical_capture_rejects_successful_rpc_token_reflection(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture, inventory_bytes, _, _ = _fixture(monkeypatch)
    secret = "historical-success-reflection-canary-a819"
    token_file = tmp_path / "rpc-token"
    token_file.write_text(secret + "\n", encoding="ascii")
    token_file.chmod(0o600)
    reflected = copy.deepcopy(fixture["raw_rpc"]["deploy"]["response"])
    reflected["id"] = 1
    reflected["result"]["api_version"] = secret
    rpc = _client(
        monkeypatch,
        _Response(json.dumps(reflected).encode("ascii")),
        authorization_files={NODE_A: token_file},
    )

    with pytest.raises(HistoricalCaptureError) as captured:
        capture_historical_odra_v1(
            proposal_payload=_proposal_payload(fixture["card_chain"]),
            rpc=rpc,
            captured_at=fixture["captured_at"],
...
def test_historical_capture_cli_threads_endpoint_scoped_authorization_file(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture, inventory_bytes, _, _ = _fixture(monkeypatch)
    ...
    proposal_path.write_text(
        json.dumps(_proposal_payload(fixture["card_chain"])),
        encoding="utf-8",
    )
    inventory_path = tmp_path / "inventory.json"
    inventory_path.write_bytes(inventory_bytes)
    token_file = tmp_path / "rpc-token"
    token_file.write_text("not-read-by-factory\n", encoding="ascii")
    token_file.chmod(0o600)
    ...
    def transport_factory(
        ...
        *,
        ...
        captured["authorization_files"] = authorization_files
        return _FakeRpc(fixture["raw_rpc"])

    monkeypatch.setattr(capture_module, "PinnedHttpsJsonRpc", transport_factory)
    result = main(
        [
            "--proposal-json",
            str(proposal_path),
            ...
            "--deployment-commit",
            fixture["deployment_commit"],
            "--public-base-url",
            "https://concordia.example",
            "--inventory",
            str(inventory_path),
            "--output",
            str(output),
