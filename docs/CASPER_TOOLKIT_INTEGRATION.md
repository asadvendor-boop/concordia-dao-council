...
| SafePay Lite | The V1 proof runtime preserves a recorded native-CSPR payment and report relation for `DAO-PROP-6CB25C`. | Historical evidence only under the Concordia apex. Casper-native x402 v2 is deployed and `https://concordiadao.xyz/x402/governance-report` exposes a live public HTTP 402 challenge; the recorded `casper-transfer` settlement finalized successfully on Casper Testnet. No external facilitator service is claimed. No external provider, escrow, refund, marketplace, or continuous-service claim is made. |
...
| CSPR.click AI Agent Skill | `integrations/cspr-click/`, `/cspr-click/unsigned-receipt/{proposal_id}`, `/cspr-click/quorum-approval/{proposal_id}`, and `/cspr-click/quorum-receipt/{proposal_id}` expose wallet-ready unsigned deploys for the receipt envelope and Odra quorum path. | Browser-wallet signing is verified for the receipt and quorum exercise; the UI still fails closed for unsupported proposals or missing package configuration. |
