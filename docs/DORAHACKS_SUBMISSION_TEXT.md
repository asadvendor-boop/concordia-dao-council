...
The judge can verify the result through CSPR.live, Concordia's `/api/ipfs/{cid}` IPFS gateway, SafePay Lite recorded native-CSPR proof, downloadable proof pack, trace API, verifier script, and QR certificate.
...
## Repository And Video Fields

Fill these after publication:
...
- SafePay Lite recorded native-CSPR specialist-report evidence.
