...
Payment acceptance is exact-only and finality is proven, not assumed. `observe_safepay_v2_payment` parses the real CSPR.live `/deploys/{hash}` record, binds the exact returned deploy hash to the requested payment hash, the canonical network, the block identity, and exactly one **raw** native transfer (the raw collection length is measured before any filtering, so one valid transfer plus a malformed or extra transfer fails closed), and reads that transfer's `initiator_account_hash` (source), `to_account_hash` (payee), amount and transfer id. A CSPR.live `status=processed` is **never** converted to finality on its own: `finality_status="finalized"` requires a defined, separate observation of the containing block (`_observe_block_finality`); processed-but-unfinalized, pending, wrong-chain and wrong-deploy responses report an honest non-final status and are retried as `425 payment_not_finalized`, never settled. The provider then requires an execution result with no execution error, exact payee equality (never substring), exact amount equality (never `>=`, overpay refused), exact transfer id equal to `correlation_id`, and exact network. An exact retry returns the identical fulfillment and `response_hash` with `delivery.replay_disposition="idempotent_replay"`; the gateway never retries 400/404/409/410/422 and `shared/x402_payments.py` no longer treats HTTP 409 as retryable indexer lag anywhere (it surfaces terminal `duplicate_conflict`).

### Truth constraint

Duplicate rejection is proven only by recorded ledger observations, and `x402_provider/ledger.py::summarize_quote_evidence` counts it TRUE only when: exactly one canonical consumption exists for the quote; an append-only `cross_binding_rejected` observation with an actually observed `http_status=409` exists on the **same** `(network, payment_hash)`; that observation evidences a **genuinely different** quote/resource binding (never a same-binding "replay"); and it was observed at or after the consumption (chronology). HTTP 200, same-binding, unrelated-payment, and pre-consumption rows never contribute, and artifact booleans (for example a claimed `duplicate_proof_rejected`) are never trusted or copied — everything is recomputed from rows.
