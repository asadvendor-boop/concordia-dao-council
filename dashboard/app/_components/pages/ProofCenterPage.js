  ...
  return <>
    ...
    {activeProofTab === "safety" && <div className="proof-two-column" role="tabpanel" id="proof-tabpanel-safety" aria-labelledby="proof-tab-safety" tabIndex={0}>
      ...
      <Panel title="Adversarial Safety Demo" eyebrow="Deterministic replay">
        {safetyProof ? <div className="safety-demo-card"><div className="safety-demo-head"><Icon name="lock" size={28} /><div><strong>{safetyProof.status === "blocked" ? "Execution Blocked" : "Safety proof ready"}</strong><p>{safetyProof.summary || "Concordia proves that an altered envelope cannot bypass the deterministic gateway."}</p></div><StatusPill tone="danger" compact>Rogue action refused</StatusPill></div><div className="safety-demo-grid"><div><span>Approved allocation</span><strong>{safetyProof.approved_allocation_label || pctFromBps(safetyProof.approved_allocation_bps)}</strong></div><div><span>Attempted allocation</span><strong>{safetyProof.attempted_allocation_label || pctFromBps(safetyProof.attempted_allocation_bps)}</strong></div><div className="wide"><span>Reason</span><strong>{safetyProof.reason || "—"}</strong></div><div><span>Locke result</span><strong>{safetyProof.locke_result || "—"}</strong></div><div><span>Proof mode</span><strong>{safetyProof.proof_mode || "—"}</strong></div><div><span>Poisoned input</span><strong>{safetyProof.poisoned_input_rejected === true ? "Rejected" : safetyProof.poisoned_input_rejected === false ? "NOT rejected" : "—"}</strong></div></div></div>
    ...
    {activeProofTab === "data" && <div role="tabpanel" id="proof-tabpanel-data" aria-labelledby="proof-tab-data" tabIndex={0}>
      ...
      <div className="proof-three-column">
        <Panel title="Council reputation" eyebrow="Accountability preview">{reputation ? <div className="reputation-list">{reputation.map((item) => <div key={`${item.agent}-${item.metric}`}><Avatar profile={Object.values(PROFILES).find((profile) => profile.name === item.agent) || PROFILES.system} size="xs" status="online" /><span><strong>{item.agent}</strong><small>{item.metric}</small></span><StatusPill tone="success" compact>{item.signal}</StatusPill></div>)}</div> : <PendingNote>Reputation metrics load from the live proof payload. No counts are shown while it is unavailable.</PendingNote>}</Panel>
        <Panel title="Mercer live Casper read" eyebrow="MCP-style data source">{liveRead ? <div className="source-status-card"><StatusPill tone="success" icon="check">Live data source</StatusPill><div><span>Network</span><strong>{liveRead.network || "casper-test"}</strong></div><div><span>Block height</span><strong>{liveRead.latest_block_height || "—"}</strong></div><div><span>State root</span>{liveRead.state_root_hash ? <HashChip value={liveRead.state_root_hash} /> : <strong>—</strong>}</div><small>{liveRead.source || "Casper Node RPC / CSPR.live"}</small></div> : <PendingNote>The live Casper read loads from the gateway. No block height or state root is shown while it is unavailable.</PendingNote>}</Panel>
