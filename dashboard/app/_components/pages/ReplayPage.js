    ...
    {!data.selectedProposal ? <Panel><VerifiedRunStaticFallback /></Panel> : !cards.length ? <Panel><EmptyState title="This proposal has no replayable evidence yet" description="Select a completed run with a sealed card chain." icon="replay" /></Panel> : <>
      <Panel className="replay-stage-panel" noPadding>
        ...
        <div className="replay-main-grid"><div className="replay-current-event"><div className="replay-agent-column"><Avatar profile={profile} size="xl" /><h2>{profile.name}</h2><p>{profile.role}</p><span>{profile.framework} · {profile.model}</span><StatusPill tone={cardTone(card)} compact>{cardBadge(card)}</StatusPill></div><div className="replay-event-copy">{/* "Verified handoff" only when the observed evidence chain reports
              chain_valid === true; otherwise the handoff is honestly recorded. */}<div className="eyebrow">{chainValid ? "Verified handoff" : "Recorded handoff"} · sequence {card.sequence}</div><h2>{replayEventTitle(card)}</h2><p className="replay-event-summary"><RichText value={cardSummary(card)} /></p>{rows.length ? <div className="replay-detail-list">{rows.map((row) => <div key={row.label}><span>{row.label}</span><strong>{Array.isArray(row.value) ? row.value.join(" · ") : typeof row.value === "object" ? publicJson(row.value, 0) : <RichText value={String(row.value)} hashChips />}</strong></div>)}</div> : null}<div className="replay-integrity-note"><Icon name={chainValid ? "shield" : "clock"} size={18} /><span><strong>{chainValid ? "Publication and identity verified" : "Reconstructed from sealed evidence"}</strong><small>{chainValid ? "This event is reconstructed from sealed Gateway evidence, not a fabricated animation." : "This event is reconstructed from sealed Gateway evidence. Chain validity is asserted only when the gateway reports it."}</small></span></div></div></div>
        <aside className="replay-right-rail"><div className="replay-rail-card"><span>Current workflow state</span><strong>{CARD_LABELS[card.card_type] || titleCaseAction(card.card_type)}</strong><small>{formatDateTime(firstDefined(getCardData(card).created_at, getCardData(card).timestamp))}</small></div><div className="replay-rail-card"><span>Proposal family</span><strong>{displayFamily(proposalFamily)}</strong><small>Baseline proof uses same-family runs</small></div><div className="replay-rail-card"><span>Proposal duration</span><strong>{formatDuration(run?.total_resolution_secs)}</strong><small>{run?.handoffs ?? deriveHandoffs(cards).length} {chainValid ? "verified" : "recorded"} handoffs</small></div><div className="replay-rail-card"><span>Evidence-chain status</span><strong className={chainValid ? "success-text" : ""}>{data.evidence?.chain_valid === false ? "Verification failed" : chainValid ? "Valid and sealed" : "Validity unavailable"}</strong><small>{cards.length} ordered cards</small></div><div className="replay-rail-card"><span>Execution conflict resolution</span><strong>Exact action only</strong><small>Altered requests are blocked before side effects</small></div></aside></div>
      </Panel>
