...
if (V1_JUDGE_ROUTE.href !== CANONICAL_REVIEW_ROUTES[1]) {
  throw new Error("V1 Judge route must remain a canonical review surface.");
}
...
const DEFAULT_IPFS_GATEWAY_URL = process.env.NEXT_PUBLIC_DEFAULT_IPFS_GATEWAY_URL || `${PUBLIC_APP_ORIGIN}/api/ipfs/${DEFAULT_IPFS_CID}`;
...
const PROOF_ACTION_REGISTRY = {
  ...
  x402_risk_report: {
    id: "x402_risk_report",
    label: "Recorded V1 SafePay Lite Evidence",
    icon: "activity",
    status: "secondary",
    tooltip: "Open recorded V1 native-CSPR SafePay Lite evidence.",
...
function useConcordiaData() {
  ...
  const forceCanonicalView = CANONICAL_REVIEW_ROUTES.some((route) => pathname?.endsWith(route));
  ...
  useEffect(() => {
    ...
    const selected = forceCanonicalView && !requested && canonical
      ? canonical.proposal_id
      : requested && (requestedExists || explicitQuorumDemo)
        ? requested
        : (canonical || active || terminal || proposals[0])?.proposal_id;
    if (selected) setSelectedId(selected);
  }, [forceCanonicalView, pathname, proposals]);
  useEffect(() => {
  ...
  return <div className={cx("app-shell", recordingMode && "recording-mode", sidebarCollapsed && "sidebar-collapsed")}>
    <aside className={cx("sidebar", mobileOpen && "sidebar-open")} aria-hidden={recordingMode ? "true" : undefined}>
      <div className="sidebar-top">
        <Link className="sidebar-brand-link" href="/landing" aria-label="Open Concordia landing page"><ConcordiaMark compact={sidebarCollapsed} /></Link>
  ...
  return <>
    <section className="overview-hero hero-glow">
      ...
      <div className="overview-hero-actions">
        <PrimaryButton icon="challenge" href={navHref("/judge", DEFAULT_REVIEW_PROPOSAL_ID)} dataTestId="overview-primary-judge">Start Judge Walkthrough</PrimaryButton>
        ...
        <PrimaryButton tone="ghost" icon="external" href={DEFAULT_CASPER_EXPLORER_URL} target="_blank" dataTestId="overview-primary-cspr">View Canonical CSPR.live Receipt</PrimaryButton>
      </div>
...
function JudgeWalkthroughPage({ data }) {
  ...
  const fallbackWalkthrough = {
    ...
    steps: [
      ...
      { step: 3, title: "Recorded V1 SafePay Lite evidence", summary: "Historical native-CSPR payment and duplicate-proof rejection evidence is preserved for review.", status: "recorded" },
    ...
    invariant_runner: { status: "passed", checks: [
      ...
      { id: "duplicate_x402_proof_rejected", label: "duplicate SafePay Lite payment proof rejected", passed: true },
  ...
  return <>
    ...
    <div className="proof-two-column">
      ...
      <Panel title="Downloads" eyebrow="Audit exports"><details className="audit-download-menu"><summary><Icon name="download" size={16} />Download audit pack</summary><div className="audit-download-menu-list"><PrimaryButton href={`${proofPackHref}/download`} icon="download">Governance archive</PrimaryButton><a href={`${proofPackHref}/exports/cards.csv`}>cards.csv</a><a href={`${proofPackHref}/exports/outcomes.csv`}>outcomes.csv</a><a href={`${proofPackHref}/exports/proof_table.csv`}>proof_table.csv</a><a href={`${proofPackHref}/exports/reputation.csv`}>reputation.csv</a><a href={`${proofPackHref}/exports/casper_receipts.csv`}>casper_receipts.csv</a><a href={`${proofPackHref}/exports/x402_settlements.csv`}>SafePay Lite recorded CSV</a></div></details></Panel>
    </div>
  </>;
}
...
function ProofCenterPage({ data }) {
  ...
  const fallbackIntegrations = {
    ...
    ipfs: {
      provider: "kubo",
      configured: true,
      gateway_base: `${PUBLIC_APP_ORIGIN}/api/ipfs`,
...
function TechnicalJuryNotePage({ data }) {
  ...
  const proofRows = [
    ...
    ["SafePay Lite (recorded V1 native-CSPR)", DEFAULT_REVIEW_PROPOSAL_ID, DEFAULT_X402_PAYMENT_HASH, "Historical payment and duplicate-proof rejection evidence"],
  ...
  return <>
    ...
    <div className="technical-note-grid">
      <Panel title="Canonical proof" eyebrow="Frozen for reproducibility">
        <p className="technical-note-lede">The canonical reviewer proof is <strong>{DEFAULT_REVIEW_PROPOSAL_ID}</strong>. It remains fixed so judges can verify the same evidence chain, Casper receipt, IPFS archive, recorded SafePay Lite proof, and certificate without the proof hierarchy shifting during review.</p>
