...
test.describe("Concordia proof cockpit browser acceptance", () => {
  ...
  test("V1.5 landing exposes truthful proof and approved publication links", async ({ page }) => {
    ...
    expect(landingHrefs.some((href) => href === "https://concordiadao.xyz/safepay-lite/DAO-PROP-6CB25C")).toBeTruthy();
    expect(landingHrefs.some((href) => /sslip\.io|https:\/\/(?:x402|safepay)\./i.test(href))).toBeFalsy();
  });

  test("grouped sidebar preserves every V1 route", async ({ page }) => {
    await gotoDashboardRoute(page, "/dashboard");
  ...
  test("landing remains usable without horizontal overflow across reviewer viewports", async ({ page }) => {
