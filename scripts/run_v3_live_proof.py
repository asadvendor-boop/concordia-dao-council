...
async def _await_finality_transcript(
    *, rpc_url: str, node_id: str, deploy_hash: str, max_attempts: int, poll_seconds: float
) -> tuple[dict[str, Any], dict[str, Any]]:
    async with httpx.AsyncClient(timeout=60.0) as client:
        for attempt in range(1, max_attempts + 1):
            params = {"deploy_hash": deploy_hash}
            request = {"jsonrpc": "2.0", "id": f"concordia-v3-finality-{attempt}", "method": "info_get_deploy", "params": params}
            response = await client.post(rpc_url, json=request)
            response.raise_for_status()
            parsed = response.json()
            if not isinstance(parsed, Mapping):
                raise LiveProofError("finality RPC response is not an object")
            if parsed.get("error") is None:
                outcome = outcome_from_finality_response(parsed)
                if outcome["finalized"]:
                    return _transcript(node_id=node_id, method="info_get_deploy", params=params, request=request, response=parsed), outcome
            if attempt < max_attempts:
                await asyncio.sleep(poll_seconds)
    raise LiveProofError(f"deploy {deploy_hash} was not finalized within the polling window")
...
def _steps(prepared: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for arg in mutated:
        if arg["name"] == "approved_allocation_bps":
            arg.update(serializer.to_json(CLV_U32(3000)))
            break
...
async def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    node_id = httpx.URL(args.rpc_url).host
    if not node_id or "@" in args.rpc_url:
        raise LiveProofError("RPC URL must identify a credential-free node")
    choreography = _steps(prepared)
    start_index = 0
    ...
    if args.resume_checkpoint is not None:
        ...
        if args.out.resolve() != args.resume_checkpoint.resolve():
            raise LiveProofError("resume must atomically update the same checkpoint path")
        checkpoint = _validate_checkpoint(
            json.loads(args.resume_checkpoint.read_text(encoding="utf-8"))
        )
        ...
        if checkpoint["status"] == "waiting_for_browser_signature":
            ...
            _persist_artifact(args.out, active_checkpoint)
        else:
    ...
    for step_index in range(start_index, len(choreography)):
        ...
        if args.prepare_only or private_key is None:
            if private_key is None and active_checkpoint is not None and step_index == start_index:
                pass
            else:
                output["status"] = (
                    "prepared" if args.prepare_only else "waiting_for_browser_signature"
                )
                ...
                _persist_artifact(args.out, checkpoint)
                return checkpoint
        if "finality_transcript" in record:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.post(args.rpc_url, json=payload)
        response.raise_for_status()
        ...
        if active_checkpoint is not None:
            ...
            _persist_artifact(args.out, output)
        finality_transcript, outcome = await _await_finality_transcript(
            rpc_url=args.rpc_url,
            node_id=node_id,
            deploy_hash=deploy_json["hash"],
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": result["status"], "artifact": str(args.out)}))
        return 0
    except (
        LiveProofError,
        ReadbackValidationError,
