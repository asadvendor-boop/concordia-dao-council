...
const consumer = mkdtempSync(path.join(tmpdir(), "concordia-verify-published-"));
writeFileSync(
  path.join(consumer, "package.json"),
  `${JSON.stringify({ name: "verify-consumer", private: true, type: "module", version: "0.0.0" })}\n`,
);
execFileSync("npm", ["install", "--ignore-scripts", "--no-audit", "--no-fund", spec], {
  cwd: consumer,
  stdio: "pipe",
});
record("clean-consumer install", true);
...
execFileSync(
  ...
  [
    "--input-type=module",
    "-e",
    "const m=await import('@concordia-dao/verify'); if(typeof m.verifyProofRegistry!=='function') process.exit(1)",
  ],
  { cwd: consumer, stdio: "pipe" },
);
record("public API import", true);

execFileSync(path.join(consumer, "node_modules/.bin/concordia-verify"), ["--help"], {
  cwd: consumer,
  stdio: "pipe",
});
record("CLI --help", true);

execFileSync("npm", ["audit", "signatures"], { cwd: consumer, stdio: "pipe" });
record("npm audit signatures", true);
